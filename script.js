...
// Paleta mais clara, com azul e creme
const PALETTE = {
  skyTop: '#dff1fb',
  skyBottom: '#f7f1e3',
  roadTint: 'rgba(230, 240, 248, 0.18)',
  sideSand: '#f3ebd7',
  laneLine: '#cfdfee',
  cream: '#f6eddc',
  creamDark: '#dfd3bc',
...
const CFG = {
  speed: 98,                    // Velocidade base do jogo
  ...
  hazardDamageMin: 1,            // Dano mínimo dos obstáculos perigosos
  hazardDamageMax: 6,            // Dano máximo dos obstáculos perigosos
...
function drawRoadBackground() {
  ...
  overlay.addColorStop(0, 'rgba(217, 235, 247, 0.16)');
  overlay.addColorStop(1, 'rgba(246, 237, 220, 0.20)');
  ctx.fillStyle = overlay;
  ctx.fillRect(0, 0, cw, ch);

  ctx.fillStyle = PALETTE.roadTint;
  ctx.fillRect(0, 0, cw, ch);
}
...
function drawCharacter(ctx, x, y, type = 'player', variant = 'normal') {
  ...
  if (type === 'player') {
    if (soldierReady) {
      ...
      ctx.fillStyle = PALETTE.blueDark; // Armadura
      ctx.beginPath();
      ctx.ellipse(0, 0, 12, 18, 0, 0, Math.PI * 2);
      ctx.fill();

      // Capacete
      ctx.fillStyle = '#eef6fd'; // Capacete claro
      ctx.beginPath();
    ...
    ctx.fillStyle = '#f0d6a6';
    ctx.beginPath();
    ctx.ellipse(0, -18, 10, 8, 0, 0, Math.PI * 2);
    ctx.fill();

    // Pluma
    ctx.fillStyle = variant === 'elite' ? '#ffe08a' : '#e98f8f';
...
class Entity {
  constructor(t, x, y, val) {
    ...
    this.damage = 0;      // Dano dos obstáculos perigosos
    this.spin = Math.random() * Math.PI * 2; // Rotação visual
  }
  ...
  draw(dy) {
    ...
    if (this.type === 'gate') {
      ...
      const postWidth = 16;
      const glow = positive ? 'rgba(111,255,111,0.55)' : 'rgba(255,111,111,0.55)';
      const body = positive ? PALETTE.bonus : PALETTE.risk;
      const bodyDark = positive ? '#2f8a68' : '#b25a5a';

      ctx.fillStyle = '#edf6fb';
      ctx.fillRect(-this.w / 2 - 12, -gateHeight / 2, postWidth, gateHeight);
      ctx.fillRect(this.w / 2 - 4, -gateHeight / 2, postWidth, gateHeight);

      ctx.fillStyle = '#bfd0df';
      ctx.fillRect(-this.w / 2 - 12, gateHeight / 2 - 16, postWidth, 16);
      ctx.fillRect(this.w / 2 - 4, gateHeight / 2 - 16, postWidth, 16);
      ...
      ctx.beginPath();
      ctx.roundRect(-this.w / 2 + 10, -this.h / 2 + 10, this.w - 20, this.h - 20, 14);
      ctx.fill();

      ctx.fillStyle = bodyDark;
      ctx.beginPath();
      ctx.roundRect(-this.w / 2 - 8, -this.h / 2 - 20, this.w + 16, 20, 8);
      ctx.fill();

      ctx.strokeStyle = positive ? '#dcffe6' : '#ffe1e1';
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.roundRect(-this.w / 2, -this.h / 2, this.w, this.h, 18);
      ctx.stroke();
      ...
      ctx.fillText(positive ? 'BÔNUS' : 'RISCO', 0, 30);

      ctx.globalAlpha = 0.2;
      ctx.fillRect(-this.w / 2 + 8, -this.h / 2 + 8, 14, this.h - 16);
      ctx.globalAlpha = 1;

      ctx.strokeStyle = 'rgba(255,255,255,0.35)';
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(0, gateHeight / 2 - 16);
      ctx.lineTo(0, gateHeight / 2 + 26);
      ctx.stroke();

      ctx.fillStyle = '#f7e6a8';
      ctx.beginPath();
      ctx.moveTo(0, gateHeight / 2 + 44);
      ctx.lineTo(-12, gateHeight / 2 + 20);
      ctx.lineTo(12, gateHeight / 2 + 20);
      ctx.closePath();
      ctx.fill();
      ...
      for (let r = -1; r <= 1; r++) {
        for (let c = -2; c <= 2; c++) {
          ctx.fillRect(c * 34 - 16, r * 24 - 10, 30, 18);
        }
      }
      ...
      ctx.fillStyle = '#ffdf6f';
      ctx.font = 'bold 20px Cinzel';
      ctx.fillText(`-${this.damage}`, 0, 82);

    } else if (this.type === 'blocker') {
      ctx.fillStyle = '#6b98c4';
      ctx.beginPath();
      ctx.roundRect(-this.w / 2, -this.h / 2, this.w, this.h, 16);
      ctx.fill();

      ctx.fillStyle = '#98c2e6';
      ctx.beginPath();
      ctx.roundRect(-this.w / 2 + 8, -this.h / 2 + 8, this.w - 16, this.h - 16, 12);
      ctx.fill();

      ctx.strokeStyle = '#f6eddc';
      ctx.lineWidth = 6;
      ctx.beginPath();
      ctx.moveTo(-this.w / 2 + 16, -this.h / 2 + 16);
      ctx.lineTo(this.w / 2 - 16, this.h / 2 - 16);
      ctx.moveTo(this.w / 2 - 16, -this.h / 2 + 16);
      ctx.lineTo(-this.w / 2 + 16, this.h / 2 - 16);
      ctx.stroke();
      ...
      ctx.shadowBlur = 24;
      ctx.shadowColor = 'rgba(84, 143, 197, 0.35)';
      ctx.fillStyle = '#f3ebdc';
      ctx.beginPath();
      ctx.roundRect(-90 * s, -80 * s, 180 * s, 180 * s, 26 * s);
      ctx.fill();

      ctx.shadowBlur = 0;
      ctx.fillStyle = PALETTE.blue;
      ctx.beginPath();
      ctx.roundRect(-72 * s, -62 * s, 144 * s, 144 * s, 22 * s);
      ctx.fill();

      ctx.fillStyle = '#f9f3e8';
      ctx.beginPath();
      ctx.arc(0, -10 * s, 32 * s, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = PALETTE.blueDark;
      ctx.fillRect(-38 * s, 20 * s, 76 * s, 46 * s);
      ...
      ctx.fillStyle = 'rgba(19, 37, 58, 0.35)';
      ctx.fillRect(-95 * s, -116 * s, 190 * s, 18 * s);
      ctx.fillStyle = PALETTE.teal;
      ctx.fillRect(-95 * s, -116 * s, 190 * s * Math.max(0, this.v / maxHp), 18 * s);
      ctx.strokeStyle = '#ffffff';
      ctx.strokeRect(-95 * s, -116 * s, 190 * s, 18 * s);

      ctx.fillStyle = PALETTE.blueDark;
      ctx.font = `bold ${Math.round(18 * s)}px Outfit`;
      ctx.textAlign = 'center';
      ctx.fillText('COMANDANTE FINAL', 0, -142 * s);
    }
  ...
  if (state === 'PLAYING') {
    ...
    entities.forEach(e => {
      ...
      if (e.type === 'cart') {
        ...
        e.swing += dt * 3;
        e.x = cx() + Math.sin(e.swing) * Math.min(120, canvas.width * 0.26); // Movimento em zigzag
        ...
        if (dx < hitW && dy < hitH && !e.passed) {
          ...
          if (h.dodge <= 0 && sShield <= 0) {
            ...
            ParticleSystem.spawn(e.x, Y, '#ff6f6f', 22, 'explosion');
            shake = 14;
            playSoundEffect('hit');
          } else {
...
function drawHouse(x, y, side) {
  ...
  ctx.shadowBlur = 8;
  ctx.shadowColor = 'rgba(255, 239, 182, 0.8)';
  ctx.fillStyle = '#ffefb6';
  ctx.fillRect(side === 'left' ? -10 : 2, -26, 8, 8);

...
const CFG = { speed: 250, eSpeed: 70, eReg: 6, weapons: { spear: { r: 80, fr: 0.2, dmg: 5, area: 0 }, bow: { r: 350, fr: 0.5, dmg: 4, area: 0 }, cannon: { r: 250, fr: 1.2, dmg: 15, area: 60 } } };
...
let h = { x: 250, y: 750, count: 10, targetX: 250, dCount: 10, units: [], vx: 0, tilt: 0, dodge: 0, weapon: 'spear', wTimers: { spear: 0, bow: 0, cannon: 0 }, cooldowns: { arrow: 0, shield: 0, fire: 0, heal: 0 } };
...
function updateHorde(n) {
    let old = h.count; h.count = Math.max(0, Math.floor(n));
...
el('skill-fire').onclick = () => useSkill('fire', 20, 15, () => {
    let k = 0; entities.forEach(e => { if (e.type === 'enemy' && e.y + dist > 0 && e.y + dist < canvas.height) { let n = Math.min(e.u.length, 20); for (let i = 0; i < n; i++) e.u.pop(); k += n; e.f = 0.3; } });
    totalKills += k; floating(250, 450, `🔥 ${k} ABATIDOS`, 'positive'); playSnd(150, 'sawtooth', 0.5, 0.1); shake = 15;
});
el('skill-heal').onclick = () => useSkill('heal', 10, 8, () => { let n = Math.floor(h.count * 0.3) + 5; updateHorde(h.count + n); floating(h.x, h.y, `+${n} 💊`, 'positive'); spawnP(h.x, h.y, '#0ff88', 20); playSnd(1000); });

document.querySelectorAll('.weapon').forEach(w => w.onclick = () => { h.weapon = w.dataset.weapon; document.querySelectorAll('.weapon').forEach(x => x.classList.remove('active')); w.classList.add('active'); playSnd(500); });
...
class Entity {
    constructor(t, x, y, val) { this.type = t; this.x = x; this.y = y; this.v = val; this.dead = false; this.u = []; this.f = 0; }
    draw(dy) {
        let Y = this.y + dy; if (Y < -200 || Y > canvas.height + 200) return;
        ctx.save(); ctx.translate(this.x, Y);
        if (this.type === 'gate') {
            ctx.fillStyle = '#5d3a1a'; ctx.fillRect(-this.x, 0, canvas.width / 2 - 5, 80); ctx.fillRect(canvas.width / 2 + 5 - this.x, 0, canvas.width / 2 - 5, 80);
            ctx.fillStyle = '#ffd966'; ctx.textAlign = 'center'; ctx.font = '24px Outfit'; ctx.fillText(this.v[0] + this.v[1], -this.x + canvas.width / 4, 50); ctx.fillText(this.v[2] + this.v[3], canvas.width * 3 / 4 - this.x, 50);
            ...
            ctx.fillStyle = '#5d3a1a'; ctx.fillRect(-this.x, 0, canvas.width, 200); ctx.fillStyle = '#c91a1a'; ctx.fillRect(-50, 20, 100, 160); ctx.fillStyle = 'white'; ctx.fillText(Math.floor(this.v), 0, -20);
        } else if (this.type === 'coin') { ctx.fillStyle = '#ffd966'; ctx.beginPath(); ctx.arc(0, 0, 12, 0, 7); ctx.fill(); }
        ctx.restore();
    }
}
...
function loadLevel(l) {
    ...
    let y = 0; for (let i = 0; i < 8; i++) {
        y -= 600; entities.push(new Entity('gate', 0, y, ['+', 10 + l, '*', 2]));
        if (Math.random() > 0.4) { let e = new Entity('enemy', 100 + Math.random() * 300, y + 300); for (let j = 0; j < 10 + l * 5; j++) e.u.push({ x: (Math.random() - 0.5) * 50, y: (Math.random() - 0.5) * 50, l: j === 0 }); entities.push(e); }
        entities.push(new Entity('coin', 50 + Math.random() * 400, y + 450));
    }
    entities.push(new Entity('boss', 250, y - 1000, 500 + l * 200));
    for (let i = 0; i < 30; i++) decors.push({ x: Math.random() > 0.5 ? 15 : 485, y: -i * 400, t: Math.random() > 0.5 ? 'lantern' : 'bamboo', s: Math.random() * 6 });
}
...
function gameLoop(t) {
    ...
    if (state === 'PLAYING') {
        ...
        projectiles = projectiles.filter(p => {
            ...
            if (p.p >= 1) {
                if (p.type === 'cannon') { shake = 10; spawnP(p.x, p.y, '#ff4444', 20, 'cannon'); entities.forEach(e => { if (e.type === 'enemy') { let d = Math.sqrt((p.x - e.x) ** 2 + (p.y - (e.y + dist)) ** 2); if (d < 110) { let k = Math.min(e.u.length, 15); for (let j = 0; j < k; j++) e.u.pop(); totalKills += k; e.f = 0.2; } } }); }
                return false;
            } return true;
        });

        entities = entities.filter(e => {
            let Y = e.y + dist; if (e.type === 'gate' && h.y > Y && h.y < Y + 80) {
                let onL = h.x < canvas.width / 2, t = onL ? e.v[0] : e.v[2], v = onL ? e.v[1] : e.v[3], n = h.count;
                if (t === '+') n += v; else if (t === '-') n -= v; else if (t === '*') n *= v; else n /= v;
                updateHorde(n); floating(h.x, h.y, (n > h.count ? '+' : '-') + Math.abs(Math.floor(n) - h.count), n > h.count ? 'positive' : 'negative'); return false;
            }
            if (e.type === 'enemy') {
                if (e.u.length === 0) return false; let d = Math.sqrt((h.x - e.x) ** 2 + (h.y - Y) ** 2);
                if (d < 100) { let k = Math.ceil(150 * dt * (h.count / 50 + 1) * (combo / 10 + 1)); for (let i = 0; i < k && e.u.length > 0; i++) { e.u.pop(); if (h.dodge <= 0) updateHorde(h.count - 1); } if (h.count <= 0) fail(); }
            ...
            if (e.type === 'boss') {
                if (h.y < Y + 200 && h.y > Y) { let d = 300 * dt * (combo / 10 + 1); e.v -= d; if (h.dodge <= 0 && sShield <= 0) updateHorde(h.count - d * 0.4); if (e.v <= 0) win(); }
            }
            if (e.type === 'coin' && Math.sqrt((h.x - e.x) ** 2 + (h.y - Y) ** 2) < 50) { coins++; energy = Math.min(100, energy + 5); playSnd(900); return false; }
            return true;
        });

        let target = entities.find(e => e.type === 'enemy' && Math.abs(e.y + dist - h.y) < CFG.weapons[h.weapon].r);
        if (target && h.wTimers[h.weapon] <= 0) {
            if (h.weapon === 'bow' && arrows > 0) { projectiles.push({ sx: h.x, sy: h.y, x: h.x, y: h.y, tx: target.x, ty: target.y + dist, p: 0, type: 'arrow', dmg: 4 }); arrows--; h.wTimers.bow = 0.5; }
            else if (h.weapon === 'cannon') { projectiles.push({ sx: h.x, sy: h.y, x: h.x, y: h.y, tx: target.x, ty: target.y + dist, p: 0, type: 'cannon', dmg: 15 }); h.wTimers.cannon = 1.2; }
        }
    ...
    ctx.fillStyle = '#3d2a1a'; ctx.fillRect(0, 0, 500, 900);
    decors.forEach(d => { let dy = d.y + dist; if (dy > -100 && dy < 1000) { d.s += 0.05; ctx.fillStyle = d.color; ctx.beginPath(); ctx.arc(d.x + Math.sin(d.s) * 8, dy, 15, 0, 7); ctx.fill(); } });
...
function updateUI() { txt.level.innerText = `⚔️ NÍVEL ${level}`; txt.coins.innerText = coins; txt.energy.innerText = Math.floor(energy); txt.arrows.innerText = arrows; Object.keys(h.cooldowns).forEach(s => { let el = document.getElementById(`skill-${s}`); if (h.cooldowns[s] > 0) { el.classList.add('cooldown'); el.querySelector('.cooldown-overlay').innerText = Math.ceil(h.cooldowns[s]) + 's'; } else el.classList.remove('cooldown'); }); }
function fail() { state = 'MENU'; screens.over.classList.add('active'); txt.kills.innerText = totalKills; txt.finalLevel.innerText = level; }
function win() { state = 'MENU'; screens.victory.classList.add('active'); let r = Math.floor(h.count * (1 + level * .2)); coins += r; txt.reward.innerText = r; }

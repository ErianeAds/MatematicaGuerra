...
// ESTADO EXPANDIDO
let state = 'MENU', lastTime = 0, level = 1, coins = 0, energy = 100, arrows = 15, totalKills = 0, dist = 0, gSpeed = CFG.speed, shake = 0, time = 0, combo = 1, comboT = 0, sShield = 0, entities = [], particles = [], projectiles = [], decors = [], floatingTexts = [], achievements = [];
let h = {
    x: 250, y: 750, count: 12, targetX: 250, dCount: 12, units: [], vx: 0, tilt: 0, dodge: 0,
    weapon: 'spear', wTimers: { spear: 0, bow: 0, cannon: 0 },
...
const updateCombo = (kills) => {
    ...
    for (let [threshold, data] of Object.entries(CFG.combos)) {
        if (combo >= threshold && !achievements.includes(`combo_${threshold}`)) {
            ...
            FloatingText.add(250, 400, `${data.name} COMBO!`, 'special');
...
el('skill-arrow').addEventListener('click', () => useSkill('arrow', 5, 3, () => {
    arrows += 15;
    floating(h.x, h.y, '+15 🏹', 'positive');
}));

el('skill-shield').addEventListener('click', () => useSkill('shield', 15, 12, () => {
    sShield = 6;
    h.armor = 2;
...
class Entity {
    ...
    draw(dy) {
        ...
        if (this.type === 'gate') {
            ...
            // Barril brilhante se estiver ativo
            if (!this.dead) {
                ctx.shadowBlur = 20;
...
function loadLevel(l) {
    ...
    // Portão principal
    gateY = -1200;
    entities.push(new Entity('gate', 250, gateY, ['+', 10 + l, '-', 2, '*', 2]));

    // Barris de poder
    for (let i = 0; i < 3; i++) {
        entities.push(new Entity('barrel', 100 + Math.random() * 300, gateY - 200 - i * 400, { dmg: 5 }));
    }

    // Power-ups especiais
    entities.push(new Entity('powerup', 150, gateY - 500, { icon: '⚡', color: '#ffff6f', buff: 'speed' }));
    entities.push(new Entity('powerup', 350, gateY - 800, { icon: '💥', color: '#ff6f6f', buff: 'crit' }));

    // Inimigos
    for (let i = 0; i < 6 + l * 3; i++) {
        let e = new Entity('enemy', 150 + Math.random() * 200, gateY - 200 - i * 280);
        let unitCount = 8 + l * 4;
    ...
    entities.push(new Entity('boss', 250, gateY - 1800, 300 + l * 250));

    // Moedas
    for (let i = 0; i < 8; i++) {
        entities.push(new Entity('coin', 50 + Math.random() * 400, gateY - 400 - i * 250));
    }

    // Decoração do cenário
    for (let i = 0; i < 25; i++) {
        decors.push({ x: 40, y: -i * 280, type: 'house', side: 'left' });
        decors.push({ x: 460, y: -i * 280, type: 'house', side: 'right' });
    }

    for (let i = 0; i < 40; i++) {
        decors.push({
            x: Math.random() > 0.5 ? 20 : 480,
            y: -i * 350,
    ...
    FloatingText.add(250, 400, `NÍVEL ${l}`, 'special');
}
...
function gameLoop(t) {
    ...
    if (state === 'PLAYING') {
        ...
        // Atualização das unidades da horda
        ...
        h.units.forEach((u, idx) => {
            ...
            // Partículas de trilha para unidades elites
        ...
        entities.forEach(e => {
            ...
            if (e.type === 'enemy') {
                ...
                if (e.burning > 0) {
                    ...
                    if (Math.random() > 0.9) {
                        e.u.pop();
                        ParticleSystem.spawn(e.x + (Math.random() - 0.5) * 50, Y + (Math.random() - 0.5) * 50, '#ff4444', 3, 'sparkle');
                    }
                }
    ...
    particles.forEach((p, i) => {
        ...
        // Gravidade para algumas partículas
        if (p.type === 'explosion') {
            p.vy += 200 * dt;
        }
...
function updateUI() {
    ...
    // Progress bar (distância até o chefe)
...
// Mostrar dica aleatória a cada 30 segundos
setInterval(() => {
    if (state === 'PLAYING') {
        let tip = tips[Math.floor(Math.random() * tips.length)];
        floating(250, 200, tip, 'positive');
    }
}, 30000);
